...
def cityscapesvideo_generator(meta, outputs, batch_size, options={ "shape": (50, 100, 3) }):
  # processed_data = process_cityscapesvideo_data(raw_dataset, options)
...
def cityscapesvideo_meta(base_path, train_val_test_split, outputs, options={}, filetype='.png', data_folders=['']):
  raw_meta = construct_cityscapesvideo_meta_dataset_from_file(base_path, filetype=filetype)
...
def cityscapesvideo(base_path, train_val_test_split, outputs):
  train_meta, val_meta, test_meta = cityscapesvideo_meta(
    base_path,
...
def construct_cityscapesvideo_meta_dataset_from_file(base_path, filetype='.png', data_folders=[''], cooked_path=''):
  ...
  for folder in full_path_raw_folders:
    current_dataframe = pd.read_csv(
      os.path.join(folder, 'telemetry_demo_video.csv'), sep=','
    )
  ...
  # Add-in filepath
  meta_dataset['Full Path'] = meta_dataset['imagePath']

  meta_dataset['filename'] = meta_dataset.apply(
    lambda r: construct_filename(r, base_path, filetype=filetype), axis=1
  )
...
def compute_cityscapesvideo_cooked_relative_path(base_path, meta_dataset, i, detect_path=False):
  video_sequence = meta_dataset.iloc[i]['videoSequence']
  folder = re.sub('demoVideo/', '', video_sequence)
  filename = meta_dataset.iloc[i]['filename']

  if (environment == 'windows'):
    folder_path = f'{base_path}\\{video_sequence}\\' #.lower()
  else:
    folder_path = f'{base_path}/{video_sequence}/' #.lower()

  data_functions.check_and_create_dir(folder_path)
  return f'{base_path}/{filename}' #.lower() # Fix Windows compat if needbe
...
def construct_filename(single_meta, base_path, filetype='.png'):
  return re.sub(base_path, '', single_meta["Full Path"])

def construct_file_path(single_meta, filetype='.png'):
  return single_meta["imagePath"]
