...
class ThreadSegmentationCooker:
  def __init__(self, batch, config, meta_dataset, dataset_string, base_path_linux):
    ...
    self.finished = False
    self.thread = Thread(name = "ThreadSegmentationCooker", target=self.internal_converter)
    ...
    while not self.finished:
      for i in self.batch:
        if i < self.meta_dataset.shape[0]:
          cooking_functions.sequential_cook_segmentation_data(i, self.config, self.meta_dataset, self.dataset_string, base_path_linux=self.base_path_linux)
