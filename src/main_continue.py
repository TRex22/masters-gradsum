...
if len(sys.argv) != 3:
  raise "Need two arguments! config_path and extra_epochs!"

config_path = sys.argv[1]
print(f'Config path: {config_path}') # Cannot log before config loaded!

extra_epochs = sys.argv[2]
...
if __name__ == '__main__':
  ...
  if (config["use_wandb"]):
    ...
    wandb_run = wandb.init(project=config["wandb_project_name"], resume=True)
  ...
  model, opt, scaler = torch_models.open_model(config, model_eval=False, epoch=start_epoch)
  ...
  train_dl, valid_dl, test_dl, train_ds, valid_ds, test_ds = data_helpers.data_processing_resume(config, dev)
  ...
  if not config["load_model"]:
    ...
    all_batch_metrics, opt, scaler, found_nan = torch_trainer.fit(extra_epochs, model, loss_func, opt, train_dl, valid_dl, config, wandb, torcheck, epoch_start=start_epoch)
