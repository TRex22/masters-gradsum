...
################################################################################
  ...
  if "compute_attention" in config:
    if config["compute_attention"]:
      config["grad_cam_ident"] = "last_attention"
  ...
  if "compute_attention" in config:
    if config["compute_attention"]:
      config["grad_cam_ident"] = "mean_last_attention"
      config["compute_mean_attention"] = True # Will compute the mean attention of all layers
      config["mean_attention_index"] = -1

      gradcam_functions.run_grad_cam(config)

  if "compute_attention" in config:
    if config["compute_attention"]:
      config["grad_cam_ident"] = "mean_first_attention"
      config["compute_mean_attention"] = True # Will compute the mean attention of all layers
      config["mean_attention_index"] = 0

      gradcam_functions.run_grad_cam(config)

  # If Using Attention then we want to test GradCAM on the CNN too
  if "compute_attention" in config:
    if config["compute_attention"]:
      config["compute_attention"] = False
      config["compute_mean_attention"] = False
      config["grad_cam_ident"] = "gradcam_cnn"
