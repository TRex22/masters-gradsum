...
# config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_threshold"
...
def run_experiment_for(epoch, config):
  ...
  config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_absolute"
  ...
  log(f'\nDefault GradCAM Time (Absolute Count): {exp_end_time - exp_start_time} secs.', config)

  config["grad_cam_algo"] = "threshold"
  ###################################################
  # Threshold 0.9
  config["grad_cam_threshold"] = 0.9
  config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_threshold_0_9"
  exp_start_time = time.time()
  ...
  gradcam_functions.run_grad_cam(config)
  exp_end_time = time.time()
  log(f'\nDefault GradCAM Time (Threshold = 0.9): {exp_end_time - exp_start_time} secs.', config)

  ###################################################
  # Threshold 0.8
  config["grad_cam_threshold"] = 0.8
  config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_threshold_0_8"
  exp_start_time = time.time()
  ...
  gradcam_functions.run_grad_cam(config)
  exp_end_time = time.time()
  log(f'\nDefault GradCAM Time (Threshold = 0.8): {exp_end_time - exp_start_time} secs.', config)

  ###################################################
  # Threshold 0.7
  config["grad_cam_threshold"] = 0.7
  config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_threshold_0_7"
  exp_start_time = time.time()
  ...
  gradcam_functions.run_grad_cam(config)
  exp_end_time = time.time()
  log(f'\nDefault GradCAM Time (Threshold = 0.7): {exp_end_time - exp_start_time} secs.', config)

  ###################################################
  # Threshold 0.6
  config["grad_cam_threshold"] = 0.6
  config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_threshold_0_6"
  exp_start_time = time.time()
  ...
  gradcam_functions.run_grad_cam(config)
  exp_end_time = time.time()
  log(f'\nDefault GradCAM Time (Threshold = 0.6): {exp_end_time - exp_start_time} secs.', config)

  ###################################################
  # Threshold 0.5
  config["grad_cam_threshold"] = 0.5
  config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_threshold_0_5"
  exp_start_time = time.time()
  ...
  gradcam_functions.run_grad_cam(config)
  exp_end_time = time.time()
  log(f'\nDefault GradCAM Time (Threshold = 0.5): {exp_end_time - exp_start_time} secs.', config)

  ###################################################
  # Threshold 0.4
  config["grad_cam_threshold"] = 0.4
  config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_threshold_0_4"
  exp_start_time = time.time()
  gradcam_functions.run_grad_cam(config)
  exp_end_time = time.time()
  log(f'\nDefault GradCAM Time (Threshold = 0.4): {exp_end_time - exp_start_time} secs.', config)
  ###################################################
  # Threshold 0.3
  config["grad_cam_threshold"] = 0.3
  config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_threshold_0_3"
  exp_start_time = time.time()
  gradcam_functions.run_grad_cam(config)
  exp_end_time = time.time()
  log(f'\nDefault GradCAM Time (Threshold = 0.3): {exp_end_time - exp_start_time} secs.', config)
  ###################################################
  # Threshold 0.2
  config["grad_cam_threshold"] = 0.2
  config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_threshold_0_2"
  exp_start_time = time.time()
  gradcam_functions.run_grad_cam(config)
  exp_end_time = time.time()
  log(f'\nDefault GradCAM Time (Threshold = 0.2): {exp_end_time - exp_start_time} secs.', config)
  ###################################################
  # Threshold 0.1
  config["grad_cam_threshold"] = 0.1
  config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_threshold_0_1"
  exp_start_time = time.time()
  gradcam_functions.run_grad_cam(config)
  exp_end_time = time.time()
  log(f'\nDefault GradCAM Time (Threshold = 0.1): {exp_end_time - exp_start_time} secs.', config)
  ##################################################
  # Threshold 0.05
  config["grad_cam_threshold"] = 0.05
  config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_threshold_0_0_5"
  exp_start_time = time.time()
  gradcam_functions.run_grad_cam(config)
  exp_end_time = time.time()
  log(f'\nDefault GradCAM Time (Threshold = 0.05): {exp_end_time - exp_start_time} secs.', config)
  ###################################################
  # Threshold 0.01
  config["grad_cam_threshold"] = 0.01
  config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_threshold_0_0_1"
  exp_start_time = time.time()
  gradcam_functions.run_grad_cam(config)
  exp_end_time = time.time()
  log(f'\nDefault GradCAM Time (Threshold = 0.01): {exp_end_time - exp_start_time} secs.', config)
  ###################################################
  # Threshold 0.00
  config["grad_cam_threshold"] = 0.00
  config["grad_cam_ident"] = f"threshold/{epoch}/grad_cam_threshold_0_0_0"
  exp_start_time = time.time()
  gradcam_functions.run_grad_cam(config)
  exp_end_time = time.time()
  log(f'\nDefault GradCAM Time (Threshold = 0.00): {exp_end_time - exp_start_time} secs.', config)
